chrome.browserAction.onClicked.addListener(function(tab) {
    ...
    var feed_url = "http://page2rss.com/page?url=" + tab.url + "&callback=page2rss_bookmark_urlr";

    chrome.browserAction.setBadgeText({text: 'wait', tabId: tab.id});

    chrome.tabs.create({"url": create_feed_url, active: false}, function(tab) {
        
        setTimeout(function() { chrome.tabs.remove(tab.id); }, 10000);
    ...
    setTimeout(function() { 
        chrome.tabs.create({"url": feed_url, active: true}, function(tab) {
            // chrome.tabs.onUpdated.addListener(function( tab.id , info ) {
            //     if ( info.status == "complete" ) {
                    // chrome.browserAction.setBadgeText({text: 'done', tabId: tab.id});
                // }
            });
